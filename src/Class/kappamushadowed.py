...
class KappaMuShadowed:
    ...
    def complex_Multipath_Fading(self):
        ...
        i = 1
        while i <= (self.mu):
            X_i = self.generate_Gaussians(xi*p_i, sigma)
            Y_i = self.generate_Gaussians(xi*q_i, sigma)
    ...
    def envelope_PDF(self):
        ...
        R = self.r / self.r_hat
        A = (2 * self.r**((2*self.mu) -1) * self.mu**self.mu * self.m**self.m * (1+self.kappa)**self.mu) / \
                (sp.gamma(self.mu) * ((self.mu*self.kappa)+self.m)**self.m * self.r_hat**(2*self.mu))
        B = np.exp(- self.mu * (1+self.kappa) * R**2)
        C = sp.hyp1f1(self.m, self.mu, ((self.mu**2 * self.kappa * (1+self.kappa))/ ((self.mu*self.kappa)+self.m)) * R**2)
