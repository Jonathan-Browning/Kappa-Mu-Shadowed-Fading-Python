...
import time
from Class.kappamushadowed import KappaMuShadowed
...
def main():   
    ...
    layout = [[sg.Text(r'Please enter \kappa, m, \mu and \hat{r}', font='Helvetica 18')],      
          [sg.Text("\kappa:", size=(8, 1), font='Helvetica 18'), sg.Input(key='-kappa', size=(5, 1), font='Helvetica 18')], 
          [sg.Text("m:", size=(8, 1), font='Helvetica 18'), sg.Input(key='-m', size=(5, 1), font='Helvetica 18')], 
    ...
    while True:
        ...
        start = time.time()
        try:
            s = KappaMuShadowed(values['-kappa'], values['-m'], values['-mu'], values['-\hat{r}'])
